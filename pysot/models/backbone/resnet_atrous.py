...
import torch
from pysot.models.attention import Spatial_attention
...
class ResNet(nn.Module):
    def __init__(self, block, layers, used_layers):
        ...
        self.self_att = Cross_attention.ATTBlock(256)
        self.sa = Spatial_attention.SpatialAttention()
    ...
    def forward(self, x):
        ...
        x_ = self.relu(x) #[40, 64, 61, 61]

        x = self.maxpool(x_) #[40, 64, 31, 31]

        p1 = self.layer1(x) #[40, 256, 31, 31]
        p1 = self.self_att(p1) * p1 + p1  # 2023.8.1 origin
        p1 = self.sa(p1)

        p2 = self.layer2(p1) #[40, 512, 15, 15]
        p3 = self.layer3(p2) #[40, 1024, 15, 15]
        p4 = self.layer4(p3) #[40, 2048, 15, 15]
